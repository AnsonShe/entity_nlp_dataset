...
def set_train_args():
    ...
    parser.add_argument("--pretrain_embed_path", type=str, default='../pretrained/tencent-ailab-embedding-zh-d200-v0.2.0-s/tencent-ailab-embedding-zh-d200-v0.2.0-s.txt', help='预训练词向量路径')
    ...
    parser.add_argument("--eval_step", type=int, default=100, help="训练多少步，查看验证集的指标")
    parser.add_argument("--max_seq_len", type=int, default=300, help="输入的最大长度")
    parser.add_argument("--max_word_num", type=int, default=3, help="每个汉字最多融合多少个词汇信息")
    parser.add_argument("--max_scan_num", type=int, default=300000, help="取预训练词向量的前max_scan_num个构造字典树")
    parser.add_argument("--data_path", type=str, default="datasets/ner_data/ner_data/ours1", help='数据集存放路径')
    parser.add_argument("--dataset_name", type=str, choices=['resume', 'ours1'], default='ours1', help='数据集名称')
    ...
    parser.add_argument("--pretrained_model_path", type=str, default="../pretrained/bert-base-chinese")
...
def main(args):
    ...
    # 初始化模型
    model = MODEL_CLASS[args.model_class].from_pretrained(args.pretrained_model_path, config=config).to(args.device)
    # 初始化模型的词向量
...
if __name__ == '__main__':
    ...
    pretrain_model = 'mengzi' if 'mengzi' in args.pretrained_model_path else 'bert-base'

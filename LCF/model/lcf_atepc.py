...
class SelfAttention(nn.Module):
    ...
    def forward(self, inputs):
        ...
        zero_tensor = torch.tensor(zero_vec).float().to('cpu')
...
class LCF_ATEPC(BertForTokenClassification):
    ...
    def get_batch_polarities(self, b_polarities):
        ...
        polarities = torch.from_numpy(polarities).long().to('cpu')
        return polarities
    ...
    def feature_dynamic_weighted(self, text_local_indices, polarities):
        ...
        return weighted_text_raw_indices.to('cpu')
    ...
    def feature_dynamic_mask(self, text_local_indices, polarities):
        ...
        return masked_text_raw_indices.to('cpu')
    ...
    def get_ids_for_local_context_extractor(self, text_indices):
        ...
        return torch.tensor(text_ids).to('cpu')
    ...
    def forward(self, input_ids_spc, token_type_ids=None, attention_mask=None, labels=None, polarities=None, valid_ids=None,
        ...
        global_valid_output = torch.zeros(batch_size, max_len, feat_dim, dtype=torch.float32).to('cpu')
        ...
        if self.args.local_context_focus is not None:
            ...
            local_context_out = self.bert_for_local_context(input_ids_spc)[0]
            ...
            local_valid_output = torch.zeros(batch_size, max_len, feat_dim, dtype=torch.float32).to('cpu')
            for i in range(batch_size):
                jj = -1
                for j in range(max_len):
                    if valid_ids[i][j].item() == 1:
